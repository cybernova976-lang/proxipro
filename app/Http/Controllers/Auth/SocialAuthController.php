...
class SocialAuthController extends Controller
{
    ...
    public function callback($provider)
    {
        try {
            ...
            $socialEmail = BlockedEmail::normalize($socialEmail);
            ...
            // Une adresse bloquée peut toujours se connecter à un compte actif existant,
            // mais elle ne peut ni recréer ni restaurer un compte supprimé.
            if ((! $user || $user->trashed())
                && (BlockedEmail::isBlocked($socialEmail) || BlockedEmail::isBlocked($user?->email))) {
                \Log::warning('Blocked email attempted social registration or restoration', [
                    'email' => $socialEmail,
                    'provider' => $provider,
