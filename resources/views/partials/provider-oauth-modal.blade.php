{{-- Modal Devenir Prestataire pour utilisateurs OAuth --}}
                ...
                <div class="oauth-step-content" id="oauthStep2">
                    ...
                    <div class="name-comparison">
                        <div class="current-name-box">
                            ...
                            <div class="current-name" id="oauthCurrentName">{{ Auth::user()->name }}</div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadFormData() {
        try {
            const response = await fetch('{{ route("become-provider.data") }}');
    ...
    document.getElementById('changeNameToggle').addEventListener('change', function() {
        ...
        if (!this.checked) {
            formData.company_name = '{{ Auth::user()->name }}';
    ...
    document.getElementById('oauth_company_name').addEventListener('input', function() {
        formData.company_name = this.value || '{{ Auth::user()->name }}';
    });
    ...
    async function submitForm() {
        ...
        try {
            const response = await fetch('{{ route("become-provider.store") }}', {
                method: 'POST',
                headers: {
