...
<style>
    ...
    /* ===== LAYOUT CENTRÉ + MENU RÉTRACTABLE ===== */
    .feed-layout {
        display: grid;
    ...
    <div id="missionsSection">
        ...
        <aside class="feed-left-menu-shell" id="feedLeftMenu">
            ...
            <div class="feed-sidepanel-content">
                ...
                @endphp
                @if($feedVerification && $feedVerification->status === 'returned')
                <a href="{{ route('verification.index') }}" style="display: block; background: linear-gradient(135deg, #fffbeb, #fef3c7); border: 2px solid #f59e0b; border-radius: 10px; padding: 14px 16px; margin-bottom: 12px; text-decoration: none; transition: transform 0.15s;">
                ...
                @elseif($feedVerification && $feedVerification->status === 'pending')
                <a href="{{ route('verification.index') }}" style="display: block; background: linear-gradient(135deg, #eff6ff, #dbeafe); border: 1px solid #93c5fd; border-radius: 10px; padding: 14px 16px; margin-bottom: 12px; text-decoration: none;">
                    <div style="display: flex; align-items: center; gap: 10px;">
                ...
                @elseif($feedVerification && $feedVerification->status === 'rejected')
                <a href="{{ route('verification.index') }}" style="display: block; background: linear-gradient(135deg, #fef2f2, #fee2e2); border: 1px solid #fca5a5; border-radius: 10px; padding: 14px 16px; margin-bottom: 12px; text-decoration: none;">
                    <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <a href="{{ route('verification.index') }}" style="display: block; background: linear-gradient(135deg, #ecfdf5, #d1fae5); border: 1px solid #86efac; border-radius: 10px; padding: 14px 16px; margin-bottom: 12px; text-decoration: none;">
                    <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <div class="sidebar-left-widgets">
                    ...
                    <div style="background: white; border: 1px solid #e2e8f0; border-radius: 14px; padding: 18px;">
                        ...
                        <div>
                            @if(isset($topPros) && $topPros->count() > 0)
                                @foreach($topPros as $index => $pro)
                                <a href="{{ route('profile.public', $pro->id) }}" style="display: flex; align-items: center; gap: 10px; padding: 8px 0; text-decoration: none; border-bottom: 1px solid #f3f4f6; {{ $index == $topPros->count() - 1 ? 'border-bottom: none;' : '' }}">
                                    ...
                                    <div style="flex: 1; min-width: 0;">
                                        <div style="font-weight: 500; color: #111827; font-size: 0.8rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $pro->name }}</div>
                                        <div style="font-size: 0.7rem; color: #9ca3af;">{{ Str::limit($pro->bio, 20) ?? 'Professionnel' }}</div>
                                    </div>
                                    <div style="color: #f59e0b; font-weight: 600; font-size: 0.75rem;">
                                        {{ number_format($pro->verified_reviews_avg ?? 0, 1, ',', '') }}
                                    </div>
                                </a>
                                @endforeach
                            @else
        ...
        <div class="feed-layout">
                ...
                <div class="create-post-card">
                    <div class="create-post-inner">
                        <div class="create-post-avatar">
                            ...
                            @endif
                            @else
                                <div class="create-post-avatar-placeholder">L</div>
                            @endauth
                        </div>
              ...
              <div class="fb-post{{ $ad->is_urgent ? ' urgent-flow' : '' }}{{ ($ad->is_boosted && $ad->boost_end && $ad->boost_end > now()) ? ' boosted-post' : '' }}{{ $photoCount > 0 ? ' has-visual' : '' }}" data-ad-id="{{ $ad->id }}"
                ...
                <div class="fb-post-header">
                    ...
                    <div class="fb-post-header-info">
                        <a href="{{ $ad->user ? route('profile.public', $ad->user->id) : '#' }}" class="fb-post-author">
                        ...
                        <div class="fb-post-meta">
                            ...
                            <span>·</span>
                            <span>{{ Str::limit($ad->location ?? 'France', 25) }}</span>
                            @if($ad->is_urgent)
                                <span>·</span>
                                <span style="color: #dc2626; font-weight: 600;"><i class="fas fa-bolt"></i> Urgent</span>
                            @endif
                            @if($ad->is_boosted && $ad->boost_end && $ad->boost_end > now())
                    ...
                    <div class="fb-post-options">
                        ...
                        <div class="fb-post-options-menu">
                            <button class="fb-post-menu-item" onclick="event.stopPropagation(); window.location.href='/profile/{{ $ad->user_id }}'">
                                <i class="fas fa-user"></i>
                                <span class="menu-item-text">Afficher le profil<small>Voir le profil de l'annonceur</small></span>
                            </button>
                            <button class="fb-post-menu-item" onclick="event.stopPropagation(); savePost({{ $ad->id }}, this)">
                                <i class="far fa-bookmark"></i>
                                <span class="menu-item-text">Enregistrer la publication<small>Ajouter à vos éléments enregistrés</small></span>
                            </button>
                            <button class="fb-post-menu-item" onclick="event.stopPropagation(); copyPostLink({{ $ad->id }})">
                                <i class="fas fa-link"></i>
                                <span class="menu-item-text">Copier le lien</span>
                            </button>
                            <div class="fb-post-menu-divider"></div>
                            <button class="fb-post-menu-item" onclick="event.stopPropagation(); hidePost({{ $ad->id }}, this)">
                                <i class="fas fa-eye-slash"></i>
                                <span class="menu-item-text">Masquer la publication<small>Voir moins de publications de ce type</small></span>
                            </button>
                            @auth
                            @if(Auth::id() === $ad->user_id)
                            <button class="fb-post-menu-item" onclick="event.stopPropagation(); window.location.href='{{ route('ads.edit', $ad->id) }}'">
                            ...
                            @endauth
                            <div class="fb-post-menu-divider"></div>
                            <button class="fb-post-menu-item danger" onclick="event.stopPropagation(); reportPost({{ $ad->id }})">
                ...
                <div class="fb-post-reactions-bar">
                    <div class="reactions-left">
                        <span>👍</span>
                        <span class="likes-count-{{ $ad->id }}">0</span>
                    </div>
                    <div class="reactions-right">
                        <span onclick="toggleComments({{ $ad->id }})"><span class="comments-count-{{ $ad->id }}">{{ $commentsCount }}</span> commentaires</span>
                ...
                <div class="fb-post-actions">
                    <button class="fb-action-btn" onclick="toggleLike({{ $ad->id }}, this)">
                        <i class="far fa-thumbs-up"></i> J'aime
                    </button>
                    @if($canReplyToAd)
                        <button class="fb-action-btn" onclick="toggleComments({{ $ad->id }})">
                            <i class="far fa-comment"></i> Commenter
                        </button>
                    @else
                        <button class="fb-action-btn" style="opacity: 0.45; cursor: not-allowed;" title="{{ $replyRestrictionMsg }}" onclick="alert('{{ $replyRestrictionMsg }}')">
                            <i class="fas fa-lock" style="font-size: 0.75rem;"></i> Commenter
                        </button>
                    @endif
                    <button class="fb-action-btn" onclick="sharePost({{ $ad->id }})">
                        <i class="fas fa-share"></i> Partager
                    </button>
                    @if($canReplyToAd)
                        <button class="fb-action-btn contact-btn" onclick="window.location.href='{{ route('ads.show', $ad->id) }}'">
                            <i class="fas fa-envelope"></i> Contacter
                        </button>
                    @else
                        <button class="fb-action-btn contact-btn" style="opacity: 0.45; cursor: not-allowed;" title="{{ $replyRestrictionMsg }}" onclick="alert('{{ $replyRestrictionMsg }}. Rendez-vous sur la page de l\'annonce pour plus de détails.')">
                ...
                <div class="fb-post-comments" id="comments-section-{{ $ad->id }}" style="display: none;">
                    <div class="fb-comments-list" id="comments-list-{{ $ad->id }}">
                        <div class="no-comments-msg" id="no-comments-{{ $ad->id }}">
                    ...
                    <form class="fb-comment-form" onsubmit="submitComment(event, {{ $ad->id }})">
                        ...
                        <div class="fb-comment-input-wrap">
                            ...
                            <button type="submit" class="fb-comment-send-btn" id="comment-submit-{{ $ad->id }}">
                                <i class="fas fa-paper-plane"></i>
                            </button>
                        </div>
                    </form>
                    @else
                    {{-- User can't reply due to restriction --}}
                    <div style="text-align:center; padding: 10px; font-size: 0.82rem; color: #92400e; background: #fef3c7; border-radius: 6px; margin: 8px 12px;">
                        <i class="fas fa-lock me-1"></i> {{ $replyRestrictionMsg }}
                        @if($adRestriction === 'pro_only')
                            — <a href="{{ route('pro.dashboard') }}" style="color: #d97706; font-weight: 600; text-decoration: none;">Devenir Pro</a>
                        @elseif($adRestriction === 'verified_only')
                            — <a href="{{ route('verification.index') }}" style="color: #059669; font-weight: 600; text-decoration: none;">Vérifier mon profil</a>
                        @endif
                    </div>
                    @endif
                    @else
                    <div style="text-align:center; padding: 8px; font-size: 0.85rem; color: #65676b;">
                        <a href="{{ route('login') }}" style="color: #1877f2; text-decoration: none; font-weight: 600;">Connectez-vous</a> pour commenter
                    </div>
                    @endauth
                </div>
            </div>
            ...
            <input type="hidden" id="currentPage" value="{{ $ads->currentPage() }}">
            <input type="hidden" id="lastPage" value="{{ $ads->lastPage() }}">
...
@include('partials.pro-onboarding-modal')

{{-- ===== POPUP CATÉGORIES MULTI-STEP ===== --}}
<div class="category-popup-overlay" id="categoryPopupOverlay" onclick="if(event.target===this) closeCategoryPopup()">
    <div class="category-popup">
        ...
        <div class="category-popup-step3" id="categoryPopupStep3" style="display:none;">
            {{-- Step 3 : description du besoin --}}
        </div>
    </div>
</div>

{{-- ===== POPUP PROPOSER MES SERVICES — 4 étapes ===== --}}
<div class="sub-popup-overlay" id="subPopupOverlay" onclick="if(event.target===this) closeSubPopup()">
    <div class="sub-popup" style="max-width: 540px;">
        ...
        <div class="sub-popup-body" id="subPopupBody">
            ...
            {{-- ═══ Étape 1 : Compléter / Récapituler le profil ═══ --}}
                ...
                <div style="max-height: 380px; overflow-y: auto; padding-right: 4px;">
                    ...
                    <div class="wizard-field-row">
                        ...
                        <div class="wizard-field-body">
                            ...
                            <input type="text" id="wizProfileName" class="wizard-field-input" value="{{ $user->name ?? '' }}" placeholder="Votre nom ou raison sociale">
                        </div>
                    </div>
                    ...
                    <div class="wizard-field-row">
                        ...
                        <div class="wizard-field-body">
                            ...
                            <input type="email" class="wizard-field-input" value="{{ $user->email ?? '' }}" disabled style="opacity:0.6; cursor:not-allowed;">
                        </div>
                    </div>
                    ...
                    <div class="wizard-field-row">
                        ...
                        <div class="wizard-field-body">
                            ...
                            <input type="text" id="wizProfileAddress" class="wizard-field-input" value="{{ $user->address ?? '' }}" placeholder="Votre adresse complète">
                        </div>
                    </div>
                    ...
                    <div class="wizard-field-row" style="align-items:flex-start;">
                        ...
                        <div class="wizard-field-body">
                            ...
                            <textarea id="wizProfileBio" class="wizard-field-input" rows="2" placeholder="Décrivez votre activité en quelques mots..." style="resize:vertical; min-height:48px;">{{ $user->bio ?? '' }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="wizard-field-row">
                        ...
                        <div class="wizard-field-body">
                            ...
                            <div style="display:flex; align-items:center; gap:8px;">
                                <span id="wizGpsStatus" style="font-size:0.78rem; color:{{ (!empty($user->latitude)) ? '#10b981' : '#ef4444' }}; font-weight:600;">
                                    {{ (!empty($user->latitude)) ? '✓ Activée' : '✗ Non activée' }}
                                </span>
            ...
            {{-- ═══ Étape 2 : Catégorie & sous-catégorie d'activité ═══ --}}
            <div id="subPopupStep2" style="display:none;">
                ...
                <div id="subPopupCategoriesTree" style="max-height: 310px; overflow-y: auto; padding-right: 4px;">
                    {{-- Built by JS: expandable categories with subcategories --}}
                </div>
            ...
            {{-- ═══ Étape 3 : Paramètres de notification ═══ --}}
            ...
            {{-- ═══ Étape 4 : Booster son profil via abonnement ═══ --}}
...
<script>
     ...
     * Utilise le disque par défaut configuré (FILESYSTEM_DISK) pour être cohérent
     * avec les controllers qui uploadent vers ce même disque.
     */
    const storageBaseUrl = @json(rtrim(config('filesystems.disks.' . config('filesystems.default', 'public') . '.url') ?: url('/storage'), '/'));
        ...
        try {
            ...
            if (data.success && data.comments && data.comments.length > 0) {
                ...
                data.comments.forEach(comment => {
                    ...
                    const deleteBtn = comment.user.id == {{ Auth::id() ?? 'null' }} 
    ...
    function openSubPopup(startStep) {
        ...
        if (notifEmail) notifEmail.checked = {{ Auth::user()->pro_notifications_email ? 'true' : 'false' }};
        if (notifRealtime) notifRealtime.checked = {{ Auth::user()->pro_notifications_realtime ? 'true' : 'false' }};
        if (notifSms) notifSms.checked = {{ Auth::user()->pro_notifications_sms ? 'true' : 'false' }};
        @endauth
    ...
    function submitProRegistration() {
        ...
        fetch('{{ route("service-provider.register") }}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' },
            body: JSON.stringify({
                services: services,
    ...
    const categoriesData = @json($missionCategories);
    ...
    function selectFeedScope(scope) {
        ...
        const nextScope = scope === 'all' ? 'all' : 'nearby';
        currentFilters.scope = nextScope;
    ...
    function changeGeoRadius(radius) {
        ...
        fetch('{{ route("feed.update-radius") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function loadMorePosts() {
            ...
            try {
                ...
                const response = await fetch(`{{ route('feed.filter-ads') }}?${params.toString()}`, {
        ...
        @if(($geoEnabled ?? false) && in_array($geoSource ?? '', ['ip', 'default']))
