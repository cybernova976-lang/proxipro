...
<div class="row g-4">
    <div class="col-xl-5">
        ...
        <form id="edit-email" action="{{ route('admin.emails.update', $managedEmail) }}" method="POST" enctype="multipart/form-data">
            @csrf @method('PUT')
    ...
    <div class="col-xl-7"><div class="card border-0 shadow-sm overflow-hidden"><div class="card-header bg-white fw-bold">Aperçu de vos modifications</div><iframe id="block-preview" sandbox="allow-same-origin" title="Aperçu des modifications" style="display:block;width:100%;height:600px;border:0;background:#f3f6fb;"></iframe></div><details class="mt-3"><summary>Aperçu réel du message enregistré (avant envoi)</summary><iframe title="Aperçu de l'e-mail" src="{{ route('admin.emails.preview', $managedEmail) }}" style="width:100%;height:760px;border:0;"></iframe></details></div>
</div>
<script>
document.addEventListener('DOMContentLoaded', () => {
    const editor = document.getElementById('edit-email');
    const approve = document.querySelector('form[action="{{ route('admin.emails.approve', $managedEmail) }}"] button');
    if (!approve) return;
    const changed = () => {approve.disabled = true; approve.textContent = 'Enregistrez les modifications avant de valider';};
