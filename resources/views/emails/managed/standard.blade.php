...
@if(empty($email->metadata['content_blocks']) && count($email->image_paths ?? []))
...
<tr><td style="padding:34px 30px 30px;">
    ...
    @if(!empty($email->metadata['content_blocks']))
        @foreach($email->metadata['content_blocks'] as $block)
            ...
            @elseif($block['type'] === 'image' && !empty($block['path']) && in_array($block['path'], $email->image_paths ?? [], true))
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="{{ $align }}" style="padding:0 0 20px;">
                    <img src="{{ storage_url($block['path']) }}" alt="{{ ($block['caption'] ?? '') ?: 'Illustration' }}" width="{{ $width }}" style="display:inline-block;width:{{ $width }}px;max-width:100%;height:auto;border-radius:8px;">
                    @if(!empty($block['caption']))<div style="font-size:12px;color:#64748b;margin-top:6px;">{{ $block['caption'] }}</div>@endif
                </td></tr></table>
            @endif
        @endforeach
    @else
